...
void ntp_disk_usage(TString filename, TString nametree="tree"){
  ...
  cout << endl 
       ...
       << setw(wbytes) << roundNumber(zip_sum,digitsBytes,nentries) << ' '
       << setw(wother) << "100.00" << ' '
       << setw(wother) << "-" << sep
       << setw(max_length) << "Total" << ' '
       << setw(wbytes) << roundNumber(zip_sum,digitsBytes,nentries) << ' '
  ...
  for(size_t ivar=0; ivar<allvars.size(); ivar++){
    ...
    cout << setw(max_length) << allvars[ivar].name << " "
	 << setw(wbytes) << roundNumber(allvars[ivar].zip_size,digitsBytes,nentries) << " "
    ...
    if(ivar<vars.size()){
      ...
      cout << setw(max_length) << vars[ivar].name << " "
	   << setw(wbytes) << roundNumber(vars[ivar].zip_size,digitsBytes,nentries) << " "
	   << setw(wother) << roundNumber(this_frac,2) << " "
