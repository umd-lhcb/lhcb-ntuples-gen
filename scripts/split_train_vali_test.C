...
void split_train_vali_test(TString input="l0hadron_emu_tm.root", TString split="35:35"){
    ...
    TString strain = split; strain.Remove(strain.First(":"), strain.Length());
    float ptrain = strain.Atof()/100.;
    TString svalid = split; svalid.Remove(0,svalid.First(":")+1);
    ...
    cout<<endl<<"Splitting the "<<nEntries<<" entries in "<<input
    <<" into training:validation:test as "<<split<<":"<<perctest<<endl<<endl;
    TRandom3 rand(42); float prob;
    for(long entry=0; entry < nEntries; entry++){
        prob = rand.Uniform(0, 1);
        if(prob <= ptrain) trainList->Enter(entry);
        else if(prob <= ptrain+pvalid) validList->Enter(entry);
        else testList->Enter(entry);
    ...
    trainFile->cd();
    trainFile->mkdir("TupleB0/"); trainFile->cd("TupleB0/");
    auto trainTree = inputTree->CopyTree("");
    trainFile->Write(); 
    cout<<"Wrote "<<trainName<<" with "<<trainTree->GetEntries()<<" entries"<<endl;
    trainFile->Close();

    inputTree->SetEntryList(validList);
    validFile->cd();
    validFile->mkdir("TupleB0/"); validFile->cd("TupleB0/");
    auto validTree = inputTree->CopyTree("");
    validFile->Write();
    cout<<"Wrote "<<validName<<" with "<<validTree->GetEntries()<<" entries"<<endl;
    validFile->Close();

    inputTree->SetEntryList(testList);
    testFile->mkdir("TupleB0/"); testFile->cd("TupleB0/");
    testFile->cd();
    auto testTree = inputTree->CopyTree("");
    testFile->Write(); 
    cout<<"Wrote "<<testName<<" with "<<testTree->GetEntries()<<" entries"<<endl<<endl;
    testFile->Close();
