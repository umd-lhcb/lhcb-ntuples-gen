...
void plot_trigger_wefficiencies(TString ntpName, TString var, vector<TString> trigs, TString basecut, 
                                float minx=0., float maxx=20., int nbins=40, TString outFolder="./"){
    ...
    for(unsigned ind=0; ind<trigs.size(); ind++){
        ...
        hname = "den"; totCut = basecut;
        histo[0] = new TH1D(hname, "", nbins, minx, maxx);
        ntp.Project(hname, var, totCut);
        hname = "num"; totCut = "("+basecut+")*("+trigs[ind]+")";
        histo[1] = new TH1D(hname, "", nbins, minx, maxx);
        ntp.Project(hname, var, totCut);

        // Adding overflow bins
        for(unsigned his(0); his<2; his++){
            double valLastbin = histo[his]->GetBinContent(nbins)+histo[his]->GetBinContent(nbins+1);
    ...
    TString pname = outFolder+"/"+varToPlain(var)+"-"+ntpName+"-Cut_"+varToPlain(basecut)+".png";
    can.SaveAs(pname);
