...
// Last Change: Thu Aug 19, 2021 at 03:53 PM +0200
...
Bool_t FLAG_ISO(Double_t iso_bdt1) { return iso_bdt1 < 0.15; }

// clang-format off
Bool_t FLAG_DD(Double_t iso_bdt1, Double_t iso_bdt2, Double_t iso_bdt3,
  ...
  auto pid_ok = MAX(
    IF(iso_bdt1 <= -1.1, 0.0f, iso_nnk1)*(iso_type1 == 3),
    IF(iso_bdt2 <= -1.1, 0.0f, iso_nnk2)*(iso_type2 == 3),
    IF(iso_bdt3 <= -1.1, 0.0f, iso_nnk3)*(iso_type3 == 3)
  ) > 0.2;

  auto kinematic_ok = MAX(
    iso_p1*(iso_pt1 > 0.15),
    iso_p2*(iso_pt2 > 0.15)*(iso_bdt2 > -1.1),
    iso_p3*(iso_pt3 > 0.15)*(iso_bdt3 > -1.1)
  ) > 5.0;

  return (iso_bdt1 > 0.15) && pid_ok && kinematic_ok;
}

Bool_t FLAG_DSTST(Double_t iso_bdt1, Double_t iso_bdt2,
                  Int_t iso_type1,
...
Bool_t FLAG_2OS(Double_t iso_bdt1, Double_t iso_bdt2, Double_t iso_bdt3,
...
Bool_t FLAG_1OS(Double_t iso_bdt1, Double_t iso_bdt2,
                Int_t iso_type1,
                Float_t iso_p1,
                Float_t iso_pt1,
                Int_t iso_chrg1,
