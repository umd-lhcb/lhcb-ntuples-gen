...
Vector<Double_t> FLAG_SEL_DD_MASS_DST(Double_t mu_mom_px, Double_t mu_mom_py, Double_t mu_mom_pz, Double_t mu_mom_pe, Double_t dst_px, Double_t dst_py, Double_t dst_pz, Double_t dst_pe,int mu_gdmom_id, Double_t mu_gdmom_px, Double_t mu_gdmom_py, Double_t mu_gdmom_pz, Double_t mu_gdmom_pe,Double_t b_px, Double_t b_py,Double_t b_pz,Double_t b_pe){
  
  auto Tmumomp = ROOT::Math::PxPyPzEVector(mu_mom_px, mu_mom_py, mu_mom_pz, mu_mom_pe);
  auto TDstp   = ROOT::Math::PxPyPzEVector(dst_px, dst_py, dst_pz, dst_pe);
  auto Tmugdmomp = ROOT::Math::PxPyPzEVector(mu_gdmom_px, mu_gdmom_py, mu_gdmom_pz, mu_gdmom_pe);
  auto TBp   = ROOT::Math::PxPyPzEVector(b_px, b_py, b_pz, b_pe);
  Double_t mDD = (Tmumomp+TDstp).M2();
  Double_t mX_DD = (TBp-TDstp-Tmumomp).M();
  if(ABS(mu_gdmom_id)==413 || ABS(mu_gdmom_id)==423 || ABS(mu_gdmom_id)==433)
        {
          mDD=(Tmugmomp+TDstp).M2();
	  mX_DD=(TBp-TDstp-Tmugmomp).M();
	}
  return {mDD,mX_DD};
}
...
Vector<Double_t> FLAG_SEL_DD_MASS_D0(Double_t mu_mom_px, Double_t mu_mom_py, Double_t mu_mom_pz, Double_t mu_mom_pe, Double_t d_mom_px, Double_t d_mom_py, Double_t d_mom_pz, Double_t d_mom_pe,int mu_gdmom_id, Double_t mu_gdmom_px, Double_t mu_gdmom_py, Double_t mu_gdmom_pz, Double_t mu_gdmom_pe, Double_t d_px, Double_t d_py, Double_t d_pz, Double_t d_pe, int d_mom_id,Double_t b_px, Double_t b_py,Double_t b_pz,Double_t b_pe){
  
  auto Tmumomp = ROOT::Math::PxPyPzEVector(mu_mom_px, mu_mom_py, mu_mom_pz, mu_mom_pe);
  auto TDmomp   = ROOT::Math::PxPyPzEVector(d_mom_px, d_mom_py, d_mom_pz, d_mom_pe);
  auto Tmugdmomp = ROOT::Math::PxPyPzEVector(mu_gdmom_px, mu_gdmom_py, mu_gdmom_pz, mu_gdmom_pe);
...
Double_t FLAG_SEL_MX_WEIGHT(Double_t b_px, Double_t b_py,Double_t b_pz,Double_t b_pe,Double_t d0_px,Double_t d0_py,Double_t d0_pz,Double_t d0_pe,Double_t d1_px,Double_t d1_py,Double_t d1_pz,Double_t d1_pe){
...
Vector<Double_t> FLAG_SEL_DALWEIGHT(Double_t mDD, int b_ID){
  ...
  if(SQRT(mDD) < 4990 )
    {
      Double_t min;
      if(ABS(b_ID)==511){min= 2010+1864.8;}//if B0 use Dst mass
      if(ABS(b_ID)==512){min= 1864.8+1864.8;}//if Bu use D0 mass
      ...
      Daltweightqp=(8*(sqrt((mDD-min2)/(max2-min2))-0.5)*(sqrt((mDD-min2)/(max2-min2))-0.5));
      Daltweightqm=(2-8*(sqrt((mDD-min2)/(max2-min2))-0.5)*(sqrt((mDD-min2)/(max2-min2))-0.5));
    }
