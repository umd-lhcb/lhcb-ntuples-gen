...
// TODO check things against code in gitlab (again) instead of preservation repo; also change the links
...
// truth-matching via OOP, so that you only have to pass the variables once.
// This template class should never be used for truth-matching, but it's useful to organize the structure and for common variables/functions
class TruthMatch {

  protected:

    // List codes used for truthmatching (some subset to be added together based on matched decay)
    int mu=0; int tau=1;
    int d0=10; int dp=20; int dst0=30; int dstp=40;
    int dstst0=100; int dststp=200; int dHstst0=300; int dHststp=400; int dsstst=500;
    int d0st=10; int d1=20; int d1p=30; int d2st=40; int dst2S=10; int d2S=20; int d2750=30; int d3000=40;
    ...
    // useful to have a map as a reference for D** cocktails (PDG MC ID -> our code)
    map<int,int> pdg_to_code{{PDG_ID_D0st_0,d0st}, {PDG_ID_D1_0,d1}, {PDG_ID_D1p_0,d1p}, {PDG_ID_D2st_0,d2st},
                             {PDG_ID_D0st,d0st}, {PDG_ID_D1,d1}, {PDG_ID_D1p,d1p}, {PDG_ID_D2st,d2st},
                             {PDG_ID_Dst2S_0,dst2S}, {PDG_ID_D2S_0,d2S}, {FAKE_ID_D2750_0,d2750}, {FAKE_ID_D3000_0,d3000},
                             {PDG_ID_Dst2S,dst2S}, {PDG_ID_D2S,d2S}, {FAKE_ID_D2750,d2750}, {FAKE_ID_D3000,d3000},
                             {PDG_ID_D1p_s,d1p}, {PDG_ID_D2st_s,d2st}}; // Ds0*,Ds2* do not exist for us
  ...
  public:
    ...
    int truthmatch=0;
    int added=0; // variable sums up cocktail/special info while truthmatching is occuring, then gets added to truthmatch
    ...
    bool debug_dstst_all_cocktail; bool debug_dstst_higher_separate_cocktail; bool debug_dstst_s_all_cocktail; bool debug_dd_all_cocktail;
    ...
    bool DSTST_OKAY(int dstst_id) { // same between D0/D* samples
      return (  (b_expect_id==PDG_ID_B0 && VEC_OR_EQ(charged_dstst, dstst_id) && !dstst_higher)
             ...
             || (b_expect_id==PDG_ID_Bu && VEC_OR_EQ(neutral_dstst_higher, dstst_id) && dstst_higher) );
    }
    virtual bool TWO_PI()=0; // different between D0/D* samples
    virtual bool B_BKGCAT_OKAY() {return false;} // different between D0/D* samples; TODO should implement this func for D* sample too
    void DSTST_COCKTAIL_ADDED(int dstst_id) { // same between D0/D* samples
      // nominally add cockatil info for D**, Ds**, not done for D**H unless specified otherwise
      if ((!dstst_higher && b_expect_id!=PDG_ID_Bs && !debug_dstst_all_cocktail) ||
    ...
    void DSTST_TWOPI_ADDED() { // same between D0/D* samples
      ...
      if (b_expect_id!=PDG_ID_Bs && !dstst_higher && TWO_PI()) added+=dstst_twopi;
    }
...
class DstTruthMatch : public TruthMatch {
  ...
  private:
    ...
    TLorentzVector dst_mom_truep4; TLorentzVector dst_truep4; int dst_bkgcat; int d0_bkgcat; int b_bkgcat;
  ...
  public:
    ...
    // TODO: Phoebe does some redefining of her Y_BKGCAT in her AddB.C, but I've just used the b_bkgcat from the ntuple as-is
    ...
    // D**, D_H**, D_s**
    // Phoebe redoHistos_Dst.C: flagBmu/flagtaumu>0 && JustDst<1  && DstOk>0. && Btype==[b_expect_id] && Dststtype==[D** cocktail] && muPID == 1
    ...
    bool TRUTH_MATCH_DSTST() {
      ...
      if (DSTST_OKAY(dst_mom_id)) {
        ...
        if (!TWO_PI()) assert(!VEC_OR_EQ(d0st_d0st0,dst_mom_id)); // just make sure that there are no D0*->D*pi decays
      }
      ...
      // If D**H, will always go as D**H->D* for D* sample reconstruction (TODO maybe worth adding an assert statement to be sure of this)
    ...
    // DD
    // Phoebe's redoHistos_Dst.C: DstOk>0. && muPID==1 && Btype==[b_expect_id] && ((flagDoubleD>0 && flagTauonicD<1) OR flagTauonicD>0)
    // Truthfully, though confusing, her truth-matching seems somewhat minimalistic/loose to me; perhaps for the best?
    ...
    DstTruthMatch(int decay_id_, int mu_id_, int mu_mom_id_, int mu_gdmom_id_, int mu_gdgdmom_id_, int mu_mom_key_, int mu_gdmom_key_, int mu_gdgdmom_key_,
                  ...
                  double dst_truePy_, double dst_truePz_, int dst_bkgcat_, int d0_bkgcat_, int b_bkgcat_,
                  bool debug_dstst_all_cocktail_, bool debug_dstst_higher_separate_cocktail_, bool debug_dstst_s_all_cocktail_, bool debug_dd_all_cocktail_) {
      // set simple fields; ensure all IDs are positive- not looking for wrong signs when truth-matching
      ...
      dst_bkgcat=dst_bkgcat_; d0_bkgcat=d0_bkgcat_; b_bkgcat=b_bkgcat_;
      debug_dstst_all_cocktail=debug_dstst_all_cocktail_; debug_dstst_higher_separate_cocktail=debug_dstst_higher_separate_cocktail_;
      debug_dstst_s_all_cocktail=debug_dstst_s_all_cocktail_; debug_dd_all_cocktail=debug_dd_all_cocktail_;

      // set decay descriptors and call truth-matching
      if (COMMON_SELEC()) { // make sure some common selections are fulfilled first
        switch (decay_id) {
          case 11574021: tau_expect=false; if (TRUTH_MATCH_NORMSIG()) truthmatch=dstp+mu; break;
          case 11574011: tau_expect=true; if (TRUTH_MATCH_NORMSIG()) truthmatch=dstp+tau; break;
          case 11874430: b_expect_id=PDG_ID_B0; tau_expect=false; dstst_higher=false; if (TRUTH_MATCH_DSTST()) truthmatch=dststp+mu+added; break;
          case 11874440: b_expect_id=PDG_ID_B0; tau_expect=true; dstst_higher=false; if (TRUTH_MATCH_DSTST()) truthmatch=dststp+tau+added; break;
          case 12873450: b_expect_id=PDG_ID_Bu; tau_expect=false; dstst_higher=false; if (TRUTH_MATCH_DSTST()) truthmatch=dstst0+mu+added; break;
          case 12873460: b_expect_id=PDG_ID_Bu; tau_expect=true; dstst_higher=false; if (TRUTH_MATCH_DSTST()) truthmatch=dstst0+tau+added; break;
          case 11676012: b_expect_id=PDG_ID_B0; tau_expect=false; dstst_higher=true; if (TRUTH_MATCH_DSTST()) truthmatch=dHststp+mu+added; break;
          case 12675402: b_expect_id=PDG_ID_Bu; tau_expect=false; dstst_higher=true; if (TRUTH_MATCH_DSTST()) truthmatch=dHstst0+mu+added; break;
          case 13674000: b_expect_id=PDG_ID_Bs; tau_expect=false; dstst_higher=false; if (TRUTH_MATCH_DSTST()) truthmatch=dsstst+mu+added; break;
          case 11894610: b_expect_id=PDG_ID_B0; tau_expect=false; if (TRUTH_MATCH_DD()) truthmatch=dd_bd+mu+added; break;
          case 12895400: b_expect_id=PDG_ID_Bu; tau_expect=false; if (TRUTH_MATCH_DD()) truthmatch=dd_bu+mu+added; break;
          case 11894210: b_expect_id=PDG_ID_B0; tau_expect=true; if (TRUTH_MATCH_DD()) truthmatch=dd_bd+tau+added; break;
          case 12895000: b_expect_id=PDG_ID_Bu; tau_expect=true; if (TRUTH_MATCH_DD()) truthmatch=dd_bu+tau+added; break;
        }
      }
    }
...
class D0TruthMatch : public TruthMatch {
  ...
  public:
    ...
    // Requires D0 has B either as gdmom or gdgdmom (TODO think about this to ensure it's right...)
    ...
    void SET_BTYPE_DSTSTTYPE() {
      ...
      if (Dststtype==0 && b_bkgcat==50 && (VEC_OR_EQ(b0_bu, mu_mom_id) || (mu_mom_id==15 && VEC_OR_EQ(b0_bu, mu_gdmom_id)))) { // Phoebe has a typo in this line (2907) I think
        Btype=b_id;
    ...
    // D**, D_H**, D_s**
    // Phoebe redoHistos_D0.C: flagBmu/flagtaumu>0 && JustDst<1 && Btype==[b_expect_id] && Dststtype==[D** cocktail] (not for D**H) && muPID==1 && simpleDstst (not for D**H,s)
    // Note: I don't implement Phoebe's ishigher, instead just using the variable dstst_higher to keep track of the considered decay ID
    // Note: Phoebe combines D**H decays into a B(+,0)->D**H->D*(+,0), a B(+,0)->D**H->D*+, and a B(+,0)->D**H->D0 template; this is implemented (at least partially) in 'added'
    ...
    bool TRUTH_MATCH_DSTST() {
      // Once the B and D** are found, make sure the D** is one of the possible cocktails for the decay mode, and check that Btype is as expected
      SET_BTYPE_DSTSTTYPE();
      ...
      if (DSTST_OKAY(Dststtype) && Btype==b_expect_id) {
        ...
        // TODO as done for D* truthmatching above, should check there are no unallowed D** decays with an assert statement
      }
      ...
      if (dstst_higher) {
        if (VEC_OR_EQ(dst_dst0, d_mom_id)) added+=dstst_higher_to_dst; // copied from how Phoebe does it
        else added+=dstst_higher_to_d; // else D**H->D
      }
    ...
    D0TruthMatch(int decay_id_, int b_id_, int mu_id_, int mu_mom_id_, int mu_gdmom_id_, int mu_gdgdmom_id_, int mu_gdgdgdmom_id_, int mu_mom_key_, int mu_gdmom_key_, int mu_gdgdmom_key_, int mu_gdgdgdmom_key_,
                  ...
                  double d_gdmom_trueE_, double d_gdmom_truePx_, double d_gdmom_truePy_, double d_gdmom_truePz_, int b_bkgcat_, double d_m_,
                  bool debug_dstst_all_cocktail_, bool debug_dstst_higher_separate_cocktail_, bool debug_dstst_s_all_cocktail_, bool debug_dd_all_cocktail_) {
      ...
      if (COMMON_SELEC()) { // make sure some common selections are fulfilled first
        switch (decay_id) {
          case 12573012: b_expect_id=PDG_ID_Bu; dst_expect_id=-1; tau_expect=false; if (TRUTH_MATCH_NORMSIG()) truthmatch=d0+mu; break;
          case 11574021: b_expect_id=PDG_ID_B0; dst_expect_id=PDG_ID_Dst; tau_expect=false; if (TRUTH_MATCH_NORMSIG()) truthmatch=dstp+mu; break;
          case 12773410: b_expect_id=PDG_ID_Bu; dst_expect_id=PDG_ID_Dst0; tau_expect=false; if (TRUTH_MATCH_NORMSIG()) truthmatch=dst0+mu; break;
          case 12573001: b_expect_id=PDG_ID_Bu; dst_expect_id=-1; tau_expect=true; if (TRUTH_MATCH_NORMSIG()) truthmatch=d0+tau; break;
          case 11574011: b_expect_id=PDG_ID_B0; dst_expect_id=PDG_ID_Dst; tau_expect=true; if (TRUTH_MATCH_NORMSIG()) truthmatch=dstp+tau; break;
          case 12773400: b_expect_id=PDG_ID_Bu; dst_expect_id=PDG_ID_Dst0; tau_expect=true; if (TRUTH_MATCH_NORMSIG()) truthmatch=dst0+tau; break;
          case 11874430: b_expect_id=PDG_ID_B0; tau_expect=false; dstst_higher=false; if (TRUTH_MATCH_DSTST()) truthmatch=dststp+mu+added; break;
          case 11874440: b_expect_id=PDG_ID_B0; tau_expect=true; dstst_higher=false; if (TRUTH_MATCH_DSTST()) truthmatch=dststp+tau+added; break;
          case 12873450: b_expect_id=PDG_ID_Bu; tau_expect=false; dstst_higher=false; if (TRUTH_MATCH_DSTST()) truthmatch=dstst0+mu+added; break;
          case 12873460: b_expect_id=PDG_ID_Bu; tau_expect=true; dstst_higher=false; if (TRUTH_MATCH_DSTST()) truthmatch=dstst0+tau+added; break;
          case 12675011: b_expect_id=PDG_ID_Bu; tau_expect=false; dstst_higher=true; if (TRUTH_MATCH_DSTST()) truthmatch=dHstst0+mu+added; break;
          case 11674401: b_expect_id=PDG_ID_B0; tau_expect=false; dstst_higher=true; if (TRUTH_MATCH_DSTST()) truthmatch=dHststp+mu+added; break;
          case 12675402: b_expect_id=PDG_ID_Bu; tau_expect=false; dstst_higher=true; if (TRUTH_MATCH_DSTST()) truthmatch=dHstst0+mu+added; break;
          case 11676012: b_expect_id=PDG_ID_B0; tau_expect=false; dstst_higher=true; if (TRUTH_MATCH_DSTST()) truthmatch=dHststp+mu+added; break;
          case 12875440: b_expect_id=PDG_ID_Bu; tau_expect=false; dstst_higher=true; if (TRUTH_MATCH_DSTST()) truthmatch=dHstst0+mu+added; break;
          case 13874020: b_expect_id=PDG_ID_Bs; tau_expect=false; dstst_higher=false; if (TRUTH_MATCH_DSTST()) truthmatch=dsstst+mu+added; break;
          case 13674000: b_expect_id=PDG_ID_Bs; tau_expect=false; dstst_higher=false; if (TRUTH_MATCH_DSTST()) truthmatch=dsstst+mu+added; break;
          case 11894600: b_expect_id=PDG_ID_B0; tau_expect=false; if (TRUTH_MATCH_DD()) truthmatch=dd_bd+mu+added; break;
          case 12893600: b_expect_id=PDG_ID_Bu; tau_expect=false; if (TRUTH_MATCH_DD()) truthmatch=dd_bu+mu+added; break;
          case 11894200: b_expect_id=PDG_ID_B0; tau_expect=true; if (TRUTH_MATCH_DD()) truthmatch=dd_bd+tau+added; break;
          case 12893610: b_expect_id=PDG_ID_Bu; tau_expect=true; if (TRUTH_MATCH_DD()) truthmatch=dd_bu+tau+added; break;
        }
      }
    }
...
// Notes for user:
// It is the user's responsibility to set decay_id correctly; there is no way for me to check this internally (unless we add a decay_id branch to ntuples elsewhere)

// TODO: since this is for user, maybe change input paramater names to match what is usually found in ntuples?
//////// Truth-Matching Selection Function (for users) for Decays Reconstructed as B0 -> D*+ [-> D0 [-> K- pi+] spi+] mu- (nu)
int MC_TRUTH_MATCH_DST(int decay_id, int mu_id, int mu_mom_id, int mu_gdmom_id, int mu_gdgdmom_id, int mu_mom_key, int mu_gdmom_key, int mu_gdgdmom_key,
                       int dst_mom_id, int dst_gdmom_id, int dst_gdgdmom_id, int dst_mom_key, int dst_gdmom_key, int dst_gdgdmom_key,
                       ...
                       double dst_truePy, double dst_truePz, int dst_bkgcat, int d0_bkgcat, int b_bkgcat, bool debug_dstst_all_cocktail=false,
                       bool debug_dstst_higher_separate_cocktail=false, bool debug_dstst_s_all_cocktail=false, bool debug_dd_all_cocktail=true) {
  ...
  DstTruthMatch event(decay_id, mu_id, mu_mom_id, mu_gdmom_id, mu_gdgdmom_id, mu_mom_key, mu_gdmom_key, mu_gdgdmom_key, dst_mom_id, dst_gdmom_id,
                      ...
                      dst_trueE, dst_truePx, dst_truePy, dst_truePz, dst_bkgcat, d0_bkgcat, b_bkgcat,
                      debug_dstst_all_cocktail, debug_dstst_higher_separate_cocktail, debug_dstst_s_all_cocktail, debug_dd_all_cocktail);
  return event.truthmatch;
}

//////// Truth-Matching Selection Function (for users) for Decays Reconstructed as B- -> D0 [-> K- pi+] mu- (nu)
int MC_TRUTH_MATCH_D0(int decay_id, int b_id, int mu_id, int mu_mom_id, int mu_gdmom_id, int mu_gdgdmom_id, int mu_gdgdgdmom_id, int mu_mom_key, int mu_gdmom_key, int mu_gdgdmom_key, int mu_gdgdgdmom_key,
                       int d_mom_id, int d_gdmom_id, int d_gdgdmom_id, int d_gdgdgdmom_id, int d_mom_key, int d_gdmom_key, int d_gdgdmom_key, int d_gdgdgdmom_key,
                       ...
                       double d_gdmom_trueE, double d_gdmom_truePx, double d_gdmom_truePy, double d_gdmom_truePz, int b_bkgcat, double d_m,
                       bool debug_dstst_all_cocktail=false, bool debug_dstst_higher_separate_cocktail=false, bool debug_dstst_s_all_cocktail=false, bool debug_dd_all_cocktail=true) {
  D0TruthMatch event(decay_id, b_id, mu_id, mu_mom_id, mu_gdmom_id, mu_gdgdmom_id, mu_gdgdgdmom_id, mu_mom_key, mu_gdmom_key, mu_gdgdmom_key, mu_gdgdgdmom_key,
                     ...
                     b_bkgcat, d_m, debug_dstst_all_cocktail, debug_dstst_higher_separate_cocktail, debug_dstst_s_all_cocktail, debug_dd_all_cocktail);
  return event.truthmatch;
}
