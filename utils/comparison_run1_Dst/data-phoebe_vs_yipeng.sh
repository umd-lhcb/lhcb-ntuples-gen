...
NTP1="../../run1-b2D0MuXB2DMuNuForTauMuLine/ntuples/run1-Dst/BCands_Dst-phoebe-data-2012-mag_down.root"
NTP2="../../run1-b2D0MuXB2DMuNuForTauMuLine/ntuples/run1-Dst/BCands_Dst-yipeng-data-2012-mag_down.root"
...
for branch in "${BRANCHES_TWO_NTP[@]}"; do
    ../plot_single_branch_two_ntuples.py -n "${NTP1}" -N "${NTP2}" \
        -t "YCands/DecayTree" -T "TupleY/DecayTree" \
        -b "${branch}" -o "${OUTPUT_DIR}/${branch}_dist.png"
done

../plot_diff_two_ntuples.py -n "${NTP1}" -N "${NTP2}" \
    -t "YCands/DecayTree" -T "TupleY/DecayTree" \
