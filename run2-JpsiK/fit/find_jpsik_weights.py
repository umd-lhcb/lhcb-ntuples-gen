...
if __name__ == '__main__':
    ...
    for year in ['2016', '2017', '2018']:
        ...
        fitFolder = f'{args.outFolder}/JpsiK-{timeStamp}-std-fit-{year}'
        runCmd(['mkdir', '-p', fitFolder])
        cmd = [fitExe, '--dataLabel', f'{year} data', '-p', 'fit/fit_params/init.yml', '-o', fitFolder, '-i']
        for ntuple in sorted(glob(f'{args.dataFolder}/*{year}*')):
            cmd.append(ntuple+':tree')
        runCmdLog(cmd, fitFolder + '/fit.log')
        fitFile = fitFolder + '/fit.root'

        ## Finding weights
        weightFile = args.outFolder + f'/run2-JpsiK-{year}-md-B-ndof_ntracks__pt_eta.root'
        cmd = [weightExe, '-d', fitFile, '-o', weightFile, '-m']
        for ntuple in sorted(glob(f'{args.mcFolder}/*{year}*')):
            cmd.append(ntuple)
