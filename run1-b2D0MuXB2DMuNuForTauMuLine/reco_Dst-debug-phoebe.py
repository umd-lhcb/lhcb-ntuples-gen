...
_MyDmup.DaughtersCuts = {
    "mu-": "(P > 3*GeV) & (MIPCHI2DV(PRIMARY)>45) & (TRCHI2DOF < 3.0)"
}  # & (PIDmu > 2)"}
...
SeqStrip = SelectionSequence(
    "SeqStrip", TopSelection=SelMyDmup
)  # , EventPreSelector=[fltr])
MyPreSelection = SeqStrip.sequence()
SeqWSStrip = SelectionSequence(
    "SeqWSStrip", TopSelection=SelMyDmum
)  # , EventPreSelector=[fltr])
...
code = (
    "(CONTAINS('"
    + SelMyDmup.outputLocation()
    + "') > 0) | (CONTAINS('"
    + SelMyDmum.outputLocation()
    + "') > 0)"
)
...
SelMyDst = Selection(
    "SelMyDst", Algorithm=_MyDst, RequiredSelections=[SelMyD0, chargedPi, upPi]
)
...
_MyWSBd.DaughtersCuts = {
    "mu+": "(PT > 100*MeV) & (P > 1*GeV) & (MIPCHI2DV(PRIMARY)>4) & (TRCHI2DOF < 3.0) & (PIDmu > 2)"
}
...
_MyB2DstPiPiPi.MotherCut = (
    "(M < 5500*MeV) & (M > 5000*MeV) & (BPVDIRA > 0.9995) & (VFASPF(VCHI2/VDOF) < 6)"
)
...
SelMyWSBd = Selection(
    "SelMyWSBd", Algorithm=_MyWSBd, RequiredSelections=[DTFSel, mulist_pre]
)
...
SeqBdMaker = SelectionSequence(
    "SeqBdMaker", EventPreSelector=[], TopSelection=SelMyB2DstPiPiPi
)
