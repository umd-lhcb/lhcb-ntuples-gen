...
if DaVinci().Simulation:
    ...
    # Bminus ###########################################################################
algo_Bminus = CombineParticles('MyBminus')
...
seq_Bminus = SelectionSequence(
    'SeqMyBminus',
    TopSelection=sel_Bminus
)
...
if DaVinci().Simulation or has_flag('CUTFLOW'):
    DaVinci().UserAlgorithms += [seq_B0.sequence(),seq_Bminus.sequence()]
else:
...
if DaVinci().Simulation or has_flag('CUTFLOW'):
    DaVinci().UserAlgorithms += [tp_B0, tp_Bminus]
else:
