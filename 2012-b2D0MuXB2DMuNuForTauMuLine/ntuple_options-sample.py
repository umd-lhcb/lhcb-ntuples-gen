...
if(controlModes==True):
    fakeLines=True


fltr = HDRFilter ('StrippedBCands', Code = "HLT_PASS('Strippingb2D0MuXB2DMuNuForTauMuLineDecision')", Location ="/Event/Strip/Phys/DecReports")
if(fakeLines):
    fltr = HDRFilter ('StrippedBCandsFAKE', Code = "HLT_PASS('Strippingb2D0MuXFakeB2DMuNuForTauMuLineDecision')", Location ="/Event/Strip/Phys/DecReports")
...
if (controlModes):
    fltr = HDRFilter ('StrippedBCands', Code = "HLT_PASS('Strippingb2D0HB2DHForTauMuLineDecision')", Location ="/Event/Strip/Phys/DecReports")
...
_smuTISfltr=FilterDesktop("smuTIS",Code="INTREE((ABSID=='mu+') & (TIS('L0.*','L0TriggerTisTos')))")
StrippedFilteredGuys=Selection(name="smuTISfilterSel",Algorithm=_smuTISfltr,RequiredSelections=[strippedGuys])
...
chargedK = Selection("chargedK",Algorithm=_chargedK,RequiredSelections=[StrippedFilteredGuys])

#pi
_chargedPi = FilterInTrees("GetPi",Code = "('pi+'==ABSID)")
chargedPi = Selection("chargedPi",Algorithm=_chargedPi,RequiredSelections=[StrippedFilteredGuys])

#mu
_mulist = FilterInTrees("GetMu",Code = "('mu+'==ABSID)")
mulist = Selection("mulist",Algorithm=_mulist,RequiredSelections=[StrippedFilteredGuys])


#D0#
_MyD0 = CombineParticles("MyD0")
_MyD0.DecayDescriptor = "[D0 -> K- pi+]cc"
_MyD0.DaughtersCuts = {"K+" : "(PT > 300*MeV) & (MIPCHI2DV(PRIMARY)>45.0) & (TRCHI2DOF < 4) & (PIDK > 4) & (TRGHOSTPROB < 0.5)",
...
_MyDst.DaughtersCuts = {"pi+" : "(MIPCHI2DV(PRIMARY)>0.0) & (TRCHI2DOF < 3) & (TRGHOSTPROB < 0.25) "}
...
SelMyDst = Selection("SelMyDst",Algorithm = _MyDst, RequiredSelections = [SelMyD0, allPi, upPi])
...
_MyDst2.DaughtersCuts = {"pi+" : "(MIPCHI2DV(PRIMARY)>0.0) & (TRCHI2DOF < 3) & (TRGHOSTPROB < 0.25) "}
...
SelMyDst2 = Selection("SelMyDst2",Algorithm = _MyDst2, RequiredSelections = [SelMyD0, allPi, upPi])

refitDst2 = FitDecayTrees("refitDst2", Code = "DECTREE('[D*(2010)- -> D0 pi-]CC')", UsePVConstraint = False, Inputs = [SelMyDst2.outputLocation()])
...
_MyB2DstPiPi.MotherCut = "(M < 5500*MeV) & (M > 5000*MeV) & (BPVDIRA > 0.9995) & (VFASPF(VCHI2/VDOF) < 9)"
...
_MyB2DstPiPiPi.MotherCut = "(M < 5500*MeV) & (M > 5000*MeV) & (BPVDIRA > 0.9995) & (VFASPF(VCHI2/VDOF) < 9)"
...
SelMyWSBd = Selection("SelMyWSBd", Algorithm = _MyWSBd, RequiredSelections = [SelMyDst, mulist])
SelMyWS2Bd = Selection("SelMyWS2Bd", Algorithm = _MyWS2Bd, RequiredSelections = [SelMyDst2, mulist])
...
SelMyB2DstPiPi = Selection("SelMyB2DstPiPi",Algorithm = _MyB2DstPiPi, RequiredSelections = [chargedPi, SelMyBd])
SelMyB2DstPiPiPi = Selection("SelMyB2DstPiPiPi",Algorithm = _MyB2DstPiPiPi, RequiredSelections = [chargedPi, SelMyBd])



SeqBuMaker = SelectionSequence('SeqBuMaker', EventPreSelector = [trigfltr,fltr], TopSelection = SelMyB2DstPiPi)
MySelectionB2DstPiPi = SeqBuMaker.sequence()
SeqBdMaker = SelectionSequence('SeqBdMaker', EventPreSelector = [trigfltr,fltr], TopSelection = SelMyB2DstPiPiPi)
...
dstpifiltsel=Selection(name='dstpifiltsel',Algorithm = _dstpifilt, RequiredSelections = [SelMyBd])
dstpifiltselseq=SelectionSequence('dstpifiltselseq',TopSelection=dstpifiltsel)

SeqYMaker = SelectionSequence('SeqYMaker', EventPreSelector = [trigfltr,fltr], TopSelection = YDTFSel)
MySelection = SeqYMaker.sequence()
SeqYMaker2 = SelectionSequence('SeqYMaker2', EventPreSelector = [trigfltr,fltr], TopSelection = YDTFSelWS)
MySelectionWS = SeqYMaker2.sequence()
SeqYMaker3 = SelectionSequence('SeqYMaker3', EventPreSelector = [trigfltr,fltr], TopSelection = YDTFSelWS2)
...
tuple1.TupleToolTISTOS.TriggerList = ['L0MuonDecision','L0HadronDecision','Hlt1TrackAllL0Decision','Hlt2CharmHadD02HH_D02KPiDecision','Hlt2DiMuonDetachedJPsiDecision','Hlt2DiMuonDetachedDecision']
...
tuple1.Y.addTool(TupleToolTagDiscardDstMu,name="MyDiscardDstMu")
tuple1.Y.ToolList+=["TupleToolTagDiscardDstMu/MyDiscardDstMu"]
tuple1.Y.addTool(TupleToolApplyIsolation, name="TTAIS")
tuple1.Y.TTAIS.WeightsFile="weightsSoft.xml"
tuple1.Y.ToolList+=["TupleToolApplyIsolation/TTAIS"]
...
tuple2.LHETT2.VOID_Variables = {
    "nTracks" : "CONTAINS ('Rec/Track/Best')"
    }
tuple2.addTupleTool("TupleToolTISTOS")
tuple2.TupleToolTISTOS.TriggerList = ['L0MuonDecision','L0HadronDecision','Hlt1TrackAllL0Decision','Hlt2CharmHadD02HH_D02KPiDecision','Hlt2DiMuonDetachedJPsiDecision','Hlt2DiMuonDetachedDecision']
...
tuple2.addBranches({
    ...
    "Kplus" : "[B0 -> (D*(2010)- -> (D~0 -> ^K+ pi-) pi-) mu-]CC",
    "muplus" : "[B0 -> (D*(2010)- -> (D~0 -> K+ pi-) pi-) ^mu-]CC"})
tuple2.Y.addTool(TupleToolTagDiscardDstMu,name="MyDiscardDstMu")
tuple2.Y.ToolList+=["TupleToolTagDiscardDstMu/MyDiscardDstMu"]
tuple2.Y.addTool(TupleToolApplyIsolation, name="TTAIS")
tuple2.Y.TTAIS.WeightsFile="weightsSoft.xml"
tuple2.Y.ToolList+=["TupleToolApplyIsolation/TTAIS"]
...
tuple3.LHETT3.VOID_Variables = {
    "nTracks" : "CONTAINS ('Rec/Track/Best')"
    }
tuple3.addTupleTool("TupleToolTISTOS")
tuple3.TupleToolTISTOS.TriggerList = ['L0MuonDecision','L0HadronDecision','Hlt1TrackAllL0Decision','Hlt2CharmHadD02HH_D02KPiDecision','Hlt2DiMuonDetachedJPsiDecision','Hlt2DiMuonDetachedDecision']
...
tuple3.addBranches({
    ...
    "Kplus" : "[B0 -> (D*(2010)+ -> (D~0 -> ^K+ pi-) pi+) mu+]CC",
    "muplus" : "[B0 -> (D*(2010)+ -> (D~0 -> K+ pi-) pi+) ^mu+]CC"})
tuple3.Y.addTool(TupleToolTagDiscardDstMu,name="MyDiscardDstMu")
tuple3.Y.ToolList+=["TupleToolTagDiscardDstMu/MyDiscardDstMu"]
tuple3.Y.addTool(TupleToolApplyIsolation, name="TTAIS")
tuple3.Y.TTAIS.WeightsFile="weightsSoft.xml"
tuple3.Y.ToolList+=["TupleToolApplyIsolation/TTAIS"]
tuple3.muplus.ToolList+=["TupleToolANNPIDTraining"]


if(controlModes==False):
    DaVinci().appendToMainSequence([scaler, MySelection, MySelectionWS, MySelectionWS2, algorithm, tuple1, tuple2, tuple3])

#if(controlModes):
#    from Configurables import PrintDecayTree
#    DaVinci().appendToMainSequence([scaler,MyFiltSelection,MySelectionB2DstPiPi,MySelectionB2DstPiPiPi,tuple,TuplePiPi, TuplePiPiPi, PrintDecayTree('PrintB',Inputs = [SeqBdMaker.outputLocation()])])
...
mcTree = PrintMCTree("PrintTrueB0")
