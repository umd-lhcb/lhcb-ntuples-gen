...
declare -A CUTS
CUTS[nnp_gt]="Brunel_MC15TuneV1_ProbNNp > 0.4 & Brunel_MC15TuneV1_ProbNNghost < 0.3"
...
rm -rf pidcalib_iso_proton

for year in 16; do
    for polarity in "up" "down"; do
        for part in "${SAMPLES[@]}"; do
            for name in "${!CUTS[@]}"; do
                folder_name="pidcalib_iso_proton/run2-rdx-20${year}-${POLARITY[${polarity}]}-${part}_${name}-p_eta_ntracks"
...
# now rename the pkls
rm -rf pkl-run2-rdx_iso_proton
mkdir -p pkl-run2-rdx_iso_proton

for pkl in ./pidcalib_iso_proton/*/*.pkl; do
    new_name="$(basename $(dirname ${pkl})).pkl"
    echo "Renaming $pkl to pkl-run2-rdx_iso_proton/${new_name}..."
    cp ${pkl} pkl-run2-rdx_iso_proton/${new_name}
done
